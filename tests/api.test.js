...
async function runTests() {
    ...
    await test('Should get todo statistics', async () => {
        ...
        assert(typeof response.body.completed === 'number', 'Should have completed count');
        assert(typeof response.body.pending === 'number', 'Should have pending count');
        assert(typeof response.body.completionRate === 'number', 'Should have completion rate');
    });
    ...
    await test('Should trim whitespace from title', async () => {
        ...
        assertEqual(response.statusCode, 201, 'Status code should be 201');
        assertEqual(response.body.title, 'Trimmed Title', 'Title should be trimmed');
    });
